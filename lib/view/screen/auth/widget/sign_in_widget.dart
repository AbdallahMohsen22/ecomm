...
class SignInWidgetState extends State<SignInWidget> {
  ...
  void loginUser() async {
    if (_formKeyLogin!.currentState!.validate()) {
      ...
      String phoneconrol = _emailController!.text.trim();
      String phone = '';
      if(phoneconrol[0] == '0'){
        phone = Provider.of<AuthProvider>(context, listen: false).countryDialCode + phoneconrol.substring(1);
      }else{
        phone = Provider.of<AuthProvider>(context, listen: false).countryDialCode + phoneconrol;
      }
      String email = phone;
      print("soly phone =========> ${email}");
